...
window.OllamaService = (() => {
  ...
  const getBase = () => {
    const configured = window.OLLAMA_BASE_URL || '';
    return configured ? normaliseUrl(configured) : '';
  };
  ...
  const showModel = async (name) => {
    return AppStream.request(endpoint(ENDPOINTS.SHOW), { name });
  };

  /* ---- Get running models ---- */
  const getRunning = async () => {
    try {
      const data = await AppStream.get(endpoint(ENDPOINTS.PS), 5000);
      return data?.models || [];
    } catch { return []; }
  };
  ...
  const chat = async (messages, options = {}) => {
    ...
    if (settings.systemPrompt) {
      ...
      if (!hasSystem) {
        body.messages.unshift({ role: 'system', content: settings.systemPrompt });
      }
    }
  ...
  const embed = async (model, input) => {
    return AppStream.request(endpoint(ENDPOINTS.EMBED), { model, input });
  };
  ...
  return {
    getBase, testConnection, listModels, showModel, getRunning,
    chat, generate, embed, abort,
  };
})();